...
func ApplyRoleCommand(ship Ship, markets_to_cover map[string]string, probe_shipyard_waypoints []Waypoint, trade_routes []TradeRoute, callsign string) {
	...
	if number_of_satellites < len(markets_to_cover) {
		...
		for _, shipyard := range probe_shipyard_waypoints {
