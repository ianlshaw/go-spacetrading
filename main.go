...
func MarketScanComplete(trade_routes []TradeRoute) bool {
	for _, trade_route := range trade_routes {
		if len(trade_route.BuyMarket.TradeGoods) == 0 {
...
func CalculateProfitPerUnit(trade_route TradeRoute) float64 {
	fmt.Println("[DEBUG] ClaculateProfitPerUnit")
	trade_good_symbol := trade_route.TradeGoodSymbol
	var sell_price float64
	for _, sell_market_trade_good := range trade_route.SellMarket.TradeGoods {
		if trade_good_symbol == sell_market_trade_good.Symbol {
			sell_price = float64(sell_market_trade_good.SellPrice)
		}
	}

	var buy_price float64
	for _, buy_market_trade_good := range trade_route.BuyMarket.TradeGoods {
		if trade_good_symbol == buy_market_trade_good.Symbol {
			buy_price = float64(buy_market_trade_good.PurchasePrice)
		}
	}
...
func ScoreTradeRoutes(ship_list []Ship, trade_routes []TradeRoute) {
	for i, trade_route := range trade_routes {
		if is_a_satellite_docked_at_marketplace(ship_list, trade_route.BuyWaypoint.Symbol) {
			if is_a_satellite_docked_at_marketplace(ship_list, trade_route.SellWaypoint.Symbol) {
				...
				//var buy_price int64
				//for j, trade_good := range trade_route.SellMarket.TradeGoods {
				//	if trade_good.Symbol == trade_routes[j].TradeGoodSymbol {
				//		buy_price = trade_good.PurchasePrice
				//	}
				//}
				//
				//var sell_price int64
				//for k, trade_good := range trade_route.BuyMarket.TradeGoods {
				//	if trade_good.Symbol == trade_routes[k].TradeGoodSymbol {
				...
				fmt.Print(" AT ")
				fmt.Print(trade_route.BuyMarket.Symbol)
				//fmt.Print(" FOR ")
				//fmt.Print(buy_price)
				fmt.Print(" SELL AT ")
				fmt.Print(trade_route.SellMarket.Symbol)
				fmt.Print(" PPU ")
				fmt.Print(trade_routes[i].ProfitPerUnit)
				fmt.Print(" DISTANCE ")
				fmt.Print(trade_routes[i].Distance)
				fmt.Print(" SCORE ")
				fmt.Print(trade_routes[i].ProfitabilityRating)
				fmt.Println()
...
func HowManyTradeGoodCanIAfford(agent Agent, trade_good_symbol string, market Market) int64 {
	var max_buy_count int64
	for _, export := range market.TradeGoods {
		if trade_good_symbol == export.Symbol {
			max_buy_count = agent.Credits / export.SellPrice
		}
	}
...
func ApplyRoleCommand(ship Ship, markets_to_cover map[string]string, probe_shipyards []Waypoint, trade_routes []TradeRoute) {
	...
	if number_of_satellites < len(markets_to_cover) {
		...
		ScoreTradeRoutes(ship_list, trade_routes)
...
func AssignSatellitesToMarkets(markets_to_cover map[string]string) {
	...
	for _, market_waypoint := range keys {
		...
		if markets_to_cover[market_waypoint] == "" {
			...
			fmt.Println("[DEBUG] Assigned satellite " + list_of_satellites[market_index].Symbol + " to market " + market_waypoint)
		}
		market_index++
	}
}
