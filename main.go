	...
	if number_of_satellites < len(markets_to_cover) {
		...
		if is_ship_cargo_empty(ship) {
			...
			if is_ship_already_at_waypoint(ship, most_profitable_trade_route.BuyMarketplaceWaypointSymbol) {
				...
				// this could potentially bankrupt the agent at very low credit values
			...
			if is_ship_already_at_waypoint(ship, most_profitable_trade_route.SellMarketplaceWaypointSymbol) {
				...
				SellCargo(ship.Symbol, most_profitable_trade_route_with_inventory_good.TradeGoodSymbol, CountTradeGoodCargo(ship, most_profitable_trade_route_with_inventory_good.TradeGoodSymbol))
				RefuelShip(ship.Symbol)
				OrbitShip(ship.Symbol)
				NavigateShip(ship.Symbol, most_profitable_trade_route.BuyMarketplaceWaypointSymbol)
			} else {
